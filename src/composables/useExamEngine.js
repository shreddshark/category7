...
const optionalQuestions = questionPool.filter((q) => !q.required)
...
export function useExamEngine() {
  ...
  function initializeExam(options = {}) {
    ...
    const requiredExamQuestions = requiredQuestions.map((question) =>
      shuffleQuestionChoices(question),
    )

    const optionalQuestionCount = Math.max(
      questionCount - requiredExamQuestions.length,
      0,
    )

    const selectedOptionalQuestions = getAdaptiveQuestions(
      optionalQuestions,
